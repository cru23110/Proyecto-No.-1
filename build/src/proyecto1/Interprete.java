...
public class Interprete {
    ...
    public void Evaluar(String expresion, int funcion) {
        if (funcion == 1) {
            ...
            saveDefun(expresion);
                ...
                && !expresion.equals("(defun fahrenheit-to-celsius (fahrenheit) (* (/ (- fahrenheit 32) 9) 5))")) {
    ...
    private boolean EvaluarCondicion(String condicion) {
        ...
        // Verifica que la condición tenga el formato esperado (por ejemplo, dos
        // operandos y un operador)
        if (partes.length != 3) {
    ...
    public String equal(String instruccion) {
        ...
        if (sonNumeros) {
            ...
            if (sonLetras) {
                // Si contiene solo letras, verifica si las variables están definidas y compara
                // sus valores
                try {
                ...
                // Si contiene letras y números, compara sus valores después de convertirlos si
                // es necesario
                try {
    ...
    public void saveDefun(String expresion) {

        Scanner scanner = new Scanner(System.in);
        System.out.print("? ");
        String input = scanner.nextLine();
        String inputLimpio = input.substring(1, input.length() - 1);
        String[] partes = inputLimpio.split("\\s+"); // Divide la expresión en partes

        if ((partes[0].equalsIgnoreCase("sumar") || partes[0].equalsIgnoreCase("add")) && partes.length == 3) {
            try {
                ...
                System.out.println("Error: Los argumentos no son números válidos.");
            }
        } else if ((partes[0].equalsIgnoreCase("restar") || partes[0].equalsIgnoreCase("subtract"))
                && partes.length == 3) {
        ...
        } else if ((partes[0].equalsIgnoreCase("multiplicar") || partes[0].equalsIgnoreCase("power"))
                && partes.length == 3) {
            try {
                int numero1 = Integer.parseInt(partes[1]);
                int numero2 = Integer.parseInt(partes[2]);
        ...
        } else if ((partes[0].equalsIgnoreCase("dividir") || partes[0].equalsIgnoreCase("divideBy")
                || partes[0].equalsIgnoreCase("divide")) && partes.length == 3) {
            try {
                int numero1 = Integer.parseInt(partes[1]);
                int numero2 = Integer.parseInt(partes[2]);
                int resultado = numero1 / numero2; // Realiza la suma
        ...
        } else if (partes[0].equalsIgnoreCase("factorial")) {
            try {
                int numero = Integer.parseInt(partes[1]);
                System.out.println("Resultado : " + factorialRecursivo(numero));
            } catch (NumberFormatException e) {
                System.out.println("Error: Los argumentos no son números válidos.");
            }

        } else if (partes[0].equalsIgnoreCase("fibonacci")) {
            try {
                int numero = Integer.parseInt(partes[1]);
                System.out.println("Resultado : " + fibonacciRecursivo(numero));
            } catch (NumberFormatException e) {
                System.out.println("Error: Los argumentos no son números válidos.");
            }

        } else if (partes[0].equalsIgnoreCase("fahrenheit-to-celsius")) {
            try {
                int numero = Integer.parseInt(partes[1]);
                System.out.println("Resultado : " + fahrenheitToCelsius(numero));
            } catch (NumberFormatException e) {
                System.out.println("Error: Los argumentos no son números válidos.");
            }

        } else {
            System.out.println("Expresión no reconocida o faltan argumentos.");
        }
    }

    public void saveCond(String expresion) {

        if (expresion
                .equalsIgnoreCase("(cond ((> n 0) (quote positivo)) ((< n 0) (quote negativo)) (T (quote cero)))")) {
            try {
